...
public class BucketOperations {
	...
	// The name of the project associated with the storage service.
	 ...
	 * @param defaultBucket The bucket default value
	 * @param projectId The default project Id 
	 */
	public static void initBucketOperations(Storage authorizedServiceClient, String projectId, String defaultBucket) {	
		...
		// Initialize default bucket name.
	...
	public static boolean createBucket(String bucketName) throws IOException {
	    ...
	    Storage.Buckets.Insert insertBucket = 
	    		buckets.insert(project, newBucket);
	 ...
	 public static void getBucket(String bucketName) throws IOException {
		...
		if (bucketName.isEmpty()) {
			...
			selectedBucketName = defaultBucketName;
		}
		else {
	  ...
	  * <p>
	  * For more information, see <a href="https://cloud.google.com/storage/docs/json_api/v1/buckets/delete" 
	  * target="_blank">Buckets:get</a>
	  * </p>
	  * @param bucketName The name of the bucket
	  * @throws IOException IO error
	 ...
	 public static void deleteBucket(String bucketName) throws IOException {
		...
		// Obtain deleteBucket method from the Buckets method collection.
		...
		displayMessageHeader("Deleting bucket " + selectedBucketName);
		
		try {
				deleteBucket = storageService.buckets().delete(selectedBucketName);
				deleteBucket.execute();
				displayMessageHeader("Bucket " + selectedBucketName + " deleteed");
		} catch (GoogleJsonResponseException e) {
		      GoogleJsonError error = e.getDetails();
		      if (error.getCode() == HTTP_CONFLICT
		          && error.getMessage().contains("You already own this bucket.")) {
